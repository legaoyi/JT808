...
public class Jt1078DeviceStateMessageHandler extends MessageHandler {
    ...
    public void handle(ExchangeMessage message) throws Exception {
        ...
        int channelId = (Integer) map.get("videoChannelId");// 推流通道号
        // 视频类型：1：实时视频；2：历史视频
        int videoType = (Boolean) map.get("isLive") ? 1 : 2;
        String cacheKey = deviceSn + "_" + channelId + "_" + videoType;
        ...
        if (state == 1) {
            ...
            cacheManager.add(cacheKey, message.getGatewayId());
        } else {
